...
\end{lbox}
\begin{note}
  Let \(X\) be the random variable such that \(X \sim \operatorname{B}(n,p)\). If \(P(X=n)\) is the \emph{highest probability} that occurs, \(X=n\) is the modal value. So, we solve the two inequalities \(P(X=5)>P(X=4)\) and \(P(X=5)>P(X=6)\). This gives the \emph{strictest} range of values that \(p\) can take (Fig [number]).
\end{note}
