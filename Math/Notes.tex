...
\begin{IN}
    \begin{itemize}[label=\footnotesize \(\square\) \normalsize]
        ...
        \item When using factor elimination to remove some \(f(x)\), we only need to say that ``\(f(x)\) is negative''\footnote{Source: Comment on Assignment A1}.
        ...
        \item Cross multiplication preserves/reverses order for \(\frac{x}{y}<\frac{x'}{y'}\) iff \(y\) and \(y'\) are \emph{both} positive or negative.\footnote{Otherwise, note the counterexample \(\frac{1}{2}<\frac{1}{-3}\).}
        \item Squaring preserves/reverses the order of \(x<y\) iff \(x\) and \(y\) are \emph{both} positive or negative.
...
\end{fact}
\begin{note}
  Sometimes computing a couple terms can be useful in finding a pattern \footnotesize(e.g. to get the coefficient of \(x^k\) explicitly)\normalsize.
\end{note}
\newpage
\section{APGP}
\begin{stbox}{Basics}{}
...
\end{stbox}
\begin{IN}
  Applications: Write out a few terms in a table and observe the trend.\footnote{You can literally say ``By observing a trend, \ldots''}
\end{IN}
\begin{lbox}[colbacktitle=white, coltitle=black, colframe=black]{G.C. Skills} 
  Table function\footnote{E.g.: By G.C., \(n \geq 182\).}
...
\begin{stbox}{Necessities}{}
  \begin{enumerate}
    ...
    \begin{enumerate}
      \item Continually expand \(u_n\) in terms of \(u_{n-1}\), then in terms of \(u_{n-2}\), \ldots, till explicit formula is obtained (e.g. \(u_1\)).
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item \(\frac{v_n}{v_{n-1}}=a\), a const. and \(\{v_n\}\) is a G.P. with 1st term \(v_1-k\) and common ratio \(a\). 
        \item So, \(v_n=(u_1-k)a^{n-1}\), and accordingly, \(u_n=v_n+k=(u_1-k)a^{n-1}+k\).

      \end{enumerate}
      \item \(\star\) Let \(u_n=Aa^n+\frac{b}{1-a}\). Then solve for the constant \(A\) with info provided.
    \end{enumerate}
...
\begin{stbox}{}
    \begin{enumerate}
      \item Formula\footnote{For \(n\) intervals (i.e. (n+1)ordinates) of width \(h:=(b-a)/n\)}: 
      \[\int_{a}^{b}y\,dx=\frac{h}{2}(y_0+2y_1+2y_2+\cdots+2y_{n-1}+y_n).\]
      \item Illustration\footnote{Trapezium rule tikzpicture credit: \url{https://tex.stackexchange.com/a/110618}}\\
    ...
    \begin{stbox}{}
      \begin{enumerate}
        \item Formula\footnote{For \(n\) intervals (i.e. (n+1)ordinates) of width \(h:=(b-a)/n\)}\footnote{Number of intervals \(n\) should be \emph{even}, that of ordinates \emph{odd}.}:
        \[\int_{a}^{b}y\,dx=\frac{h}{3}(y_0+4y_1+2y_2+4y_3+\cdots+2y_{n-2}+4y_{n-1}+y_n)\]
        \item Illustration\footnote{Simpson's rule tikzpicture credit: \url{https://tex.stackexchange.com/a/439119}}\\
        \includestandalone{../Diagrams/Simpson}
        Accuracy of the Trapezium rule vs Simpson's Rule: ``Simpson's Rule uses \emph{quadratic curves} to interpolate the points on the curve so it usually \emph{gives a better approximation} to the actual curve than the trapezium rule which uses \emph{straight lines} to interpolate the ordinates.''
      \end{enumerate}
    \end{stbox}
...
\begin{stbox}{}
  \begin{enumerate}
    ...
    \begin{enumerate}
      \item The locus represented by \(\lvert z-a \rvert =r\) (or \(z=a+re^{i\theta}\)) is a \emph{circle} of radius \(r\) centered at \(A(\operatorname{Re}(a),\operatorname{Im}(a))\).
      \begin{center}
      ...
      \begin{center}
        \includegraphics[scale=0.1]{../images/ComplexPerpendicularBisectorLocus}
      \end{center}
...
\begin{lbox}[colbacktitle=white, coltitle=black, colframe=black]{G.C. Skills} 
  Transfrm app (allows you to vary a parameter of a function\footnote{E.g.: The variable \(A\) in \(y=Ax+b\)})
\end{lbox}
\begin{IN}
\begin{center}
  \begin{tikzpicture}
...
\hline
    ...
    \begin{itemize}[label=\footnotesize\(\triangle\)]
      \item \(r\) is a function of \(\cos(n\theta)\)\footnote{E.g.: \(r=a\sqrt{(4+\sin^2(\theta))^2\cos(\theta)}\)} \emph{only} \(\implies\) lines of symmetry: \(n\theta=0,\pi,2\pi,\ldots,m\pi,\ldots\)
    \end{itemize}
    \item \(r=f(\theta)\) is symmetrical about the vertical line \(\theta=\frac{\pi}{2}\) iff the equation \(f(\theta)=f(\pi-\theta)\).
    \begin{itemize}[label=\footnotesize\(\triangle\)]
      \item \(r\) is a function of \(\sin(n\theta)\) \emph{only} \(\implies\) lines of symmetry: \(n\theta=\frac{\pi}{2},\frac{3\pi}{2},\ldots,\frac{(2m+1)\pi}{2},\ldots\)
    \end{itemize}
    \item \(r=f(\theta)\) is symmetrical about the pole iff\footnote{In other words, \(r=f(\theta)\) is unchanged when \(r\) is replaced by \(-r\).} \((r,\theta)\) is a point on the curve whenever \((-r,\theta)\) is.
...
\begin{IN}
  \begin{enumerate}
    \item \(r\) is \emph{always}\footnote{in some situations it can be negative} \(\geq 0\) in our syllabus.
...
\begin{stbox}{Essentials}{}
  \begin{itemize}[label=\(\circ\)]
    \item Eccentricity, \(e:=\frac{\text{distance of \(P\) from focus}}{\text{distance of \(P\) from directrix}}\).
    \begin{itemize}
      \item \(e=0\): Circle
      \item \(0<e<1\): Ellipse 
      \item \(e=1\): Parabola
...
\begin{stbox}{General Necessities}
  \begin{itemize}
    ...
    \item \({^n}P_k := \frac{n!}{(n-k)!}\quad\)
    \item Know how to `bundle' objects together so as to calculate the total no. of permutations.
    \item There are \(\frac{n!}{n_1!n_2!\cdots n_r!}\) number of ways to arrange \(n\) objects, of which \(n_1\) are `similar', \(n_2\) are `alike', \ldots, \(n_r\) are `the same'.\footnote{Intuition: If there are \(n_1\) objects are non-distinct out of \(n\) objects, then there are \(n_1!\) ways to arrange these objects that results in `the same' permutation.}
    \item Case-wise considerations/calculations (then summing tgt the total no of perms)
    \item Unordered circular permutations:\\
    There are \(\frac{n!}{n}=(n-1)!\) number of ways of arranging \(n\) distinct objects in a circle.\footnote{For unordered CPs, we do not care if you rotate the seating arrangement, as long as neighbours are preserved for each object. i.e. \((A,B,C,D) \sim (B,C,D,A)\). As a result, each such collection of \(n\) permutations reduces down to one. Thus, explaining the division by \(n\).}
    \item Complementary Method, i.e. taking no. of arr w/o restriction - no. of arr w/ the opposite of that restriction.\footnote{E.g. No. of ways 2 girls \emph{cnnt} sit nxt to each other = no. of arr w/o restriction - no. of arr with girls sitting \emph{tgt}. \vspace*{0.5mm}}
    \item Insertion Method, place down some of your objects and then insert the rest in the gaps.\footnote{\vspace{-4.5mm}\begin{enumerate}[label=]
    ...
    \item Ordered circular permutations: Add the ordering at the end. Note:
    \begin{enumerate}
      \item The number of ways is not necessarily just \((n-1)! \cdot n=n!\).
      \item Circular arrangements are not the same as row arrangements.\footnote{While \(A\) and \(B\) are not considered to be seating together in the row arrangement of \((A,C,D,E,B)\), they are seating together in a corresponding row arrangement. Does not always mean no. of row arr \(<\) no. of circ arr. It can be \(<\), \(=\), \(>\).}
    \end{enumerate}
    \item \(\binom{n}{r}={^n}C_r:=\frac{n!}{(n-r)!r!}\).
  \end{itemize}
\end{stbox}
\chapter{Vectors}
...
\begin{minipage}{0.5\textwidth}
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item \(\overrightarrow{QN} \cdot m=0\), solve for \(\lambda\),
...
\end{minipage}\\
\hline
\multicolumn{2}{|Sc|}{Shortest Disance of Point To Line, \(QN\)}\\
\hline
\begin{minipage}{0.5\textwidth}
  \begin{enumerate}
...
\end{minipage} &
  ...
  \multicolumn{2}{|Sc|}{
    \begin{tabular}{Sc|Sc|Sc}
      \begin{minipage}{0.333\textwidth}
        \[\theta=\cos^{-1}\left\lvert\frac{\mathbf{m_1}\cdot \mathbf{m_2}}{\norm{\mathbf{m_1}}\norm{\mathbf{m_2}}}\right\rvert.\]
      \end{minipage} &
      \begin{minipage}{0.333\textwidth} 
        \[\theta=\sin^{-1}\left\lvert \frac{\mathbf{m}\cdot \mathbf{n}}{\norm{\mathbf{m}}\norm{\mathbf{n}}} \right\rvert .\]
      \end{minipage} &
      \begin{minipage}{0.333\textwidth}
        \[\theta=\cos^{-1}\left\lvert \frac{\mathbf{n_1}\cdot \mathbf{n_2}}{\norm{\mathbf{n_1}}\norm{\mathbf{n_2}}} \right\rvert.\]
      \end{minipage}\\
    \end{tabular}
  }\\
  \hline
...
\begin{stbox}{}
  \begin{enumerate}
    \item Principle of Inclusion and Exclusion:
    \[\operatorname{P}(A \cup B)=\operatorname{P}(A)+\operatorname{P}(B)-\operatorname{P}(A \cap B),\]
    \[\operatorname{P}(A \cup B \cup C)=\operatorname{P}(A)+\operatorname{P}(B)+\operatorname{P}(C)-\operatorname{P}(A\cap B)-\operatorname{P}(A \cap C)-\operatorname{P}(B \cap C)+\operatorname{P}(A \cap B \cap C).\]
    \item Mutually Exclusive Events:
    \begin{align*}
      \operatorname{P}(A \cap B)&=0,\\
      \operatorname{P}(A \cup B)&=\operatorname{P}(A)+\operatorname{P}(B).
    \end{align*}
    \item Independent Events:
    \begin{align*}
      \operatorname{P}(A \,\vert\, B)&=\operatorname{P}(A),\\
      \operatorname{P}(A \cap B)&=\operatorname{P}(A)\operatorname{P}(B).
    \end{align*}
    \item Conditional Probability:
    \[\operatorname{P}(A \,\vert\, B)=\frac{\operatorname{P}(A \cap B)}{\operatorname{P}(B)}.\]
...
\begin{stbox}{}
  \begin{enumerate}
    ...
    \[\operatorname{E}(X):=\sum_{\text{all } x}{x\operatorname{P}(X=x)}.\]
    \item Variance 
    ...
    \begin{tabular}{|Sc|Sc|Sc|Sc|}
      ...
      \(x\) & 1 & \(\cdots\) & \(n\)\\
      \hline
      \(\operatorname{P}(X=x)\) & \(\operatorname{P}(X=1)\) & \(\cdots\) & \(\operatorname{P}(X=n)\)\\
      \hline
    \end{tabular}
  \end{enumerate}
\end{stbox}
...
\begin{definition}{}{}
  A probability distribution function of \(X\) is a binomial distribution, i.e. \(X \sim B(n,p)\) iff 
  \[P(X=x):=\binom{n}{x}p^x(1-p)^{n-x},\]
  for all \(0 \leq x \leq n\), where \(p\) is probability of success.
\end{definition}
\begin{stbox}{}
  If \(X \sim \operatorname{B}(n,p)\),
  \begin{enumerate}
    \item Expectation / Mean:
    \[\operatorname{E}(X)=np \qquad\text{and}\qquad \operatorname{Var}(X)=np(1-p).\]
  \end{enumerate}
\end{stbox}
\begin{note}
  We can assume \(X \sim \operatorname{B}(n,p)\) whenever
  \begin{enumerate}
...
\end{note}
\begin{note}
  Defining random variables: Let \(X\) be the number of [event in context], out of [number of trials \(n\) in context].  
\end{note}
\begin{note}
...
\begin{lbox}[colbacktitle=white, coltitle=black, colframe=black]{G.C. Skills} 
  \begin{enumerate}
    \item \texttt{2nd + Vars + `A' \(\implies\) binompdf\((n,p,x)=\operatorname{P}(X=x)\)}
    \item \texttt{2nd + Vars + `B' \(\implies\) binomcdf\((n,p,x)=\operatorname{P}(X\leq x)\)}
  \end{enumerate}
\end{lbox}
\begin{note}
  Let \(X\) be the random variable such that \(X \sim \operatorname{B}(n,p)\). If \(P(X=n)\) is the \emph{highest probability} that occurs, \(X=n\) is the modal value. So, we solve the two inequalities \(P(X=5)>P(X=4)\) and \(P(X=5)>P(X=6)\). This gives the \emph{strictest} range of values that \(p\) can take (Fig 17.1).
\end{note}
\begin{center}
  \includegraphics[scale=1]{BinomDist}
  \captionof{figure}{In this case, \(X=10\) is the mode.} \footnotetext{Source for Fig 17.1: \url{https://math.oxford.emory.edu/site/math117/normalApproxToBinomial/}}
\end{center}
...
\end{example}
\chapter{Foreword}
\(>\) Wait isn't the foreword supposed to be\ldots in front?\\
\(>\) \emph{Y e s}\\[5mm]
\section{Main Word}
\begin{center}
  Latex is pain,\\
