...
\begin{note}
  ...
  \begin{enumerate}
    ...
    \item Fix an odd \(n\in \mathbb{Z}^{+}\). Suppose we want to express \(\sin^n(\theta)\) as a linear combination of \(\sin(m\theta)\), for \(m\in \mathbb{Z}^{+}\). First notice that \(z^k-\frac{1}{z^k}=2i\sin(k\theta)\), for \(z=e^{i\theta}\). Then, use this fact in conjunction with the binomial theorem:
    \begin{align*}
      \left( z-\frac{1}{z} \right)^n &= \sum_{r=0}^{2n+1}{\binom{n}{r}(-1)^rz^{2n+1-2r}}\\
...
\begin{example}{RV FM CT 2024}{}
  ...
  \end{subfigure}%
    \caption{The writing in blue denote the deductions we should make.}
