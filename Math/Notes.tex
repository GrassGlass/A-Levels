  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \begin{center}
        ...
        \captionof{figure}{The locus of \(\lvert z-a \rvert =r\), a circle.}
      \end{center}
      \begin{enumerate}
        \item Either label the four points to the direct North, South, East, West of the circle, or the denote radius clearly. 
...
\begin{definition}{}{}
  A discrete random variable \(X\) which takes all integer values from \(1\) to \(n\) (inclusive) is a \emph{binomial distribution} with probability of success \(p\), denoted by \(X \sim \operatorname{B}(n,p)\), iff
...
\begin{note}
  ...
  \begin{enumerate}
    ...
    \item The \emph{mean number of occurrences} of [trial in context] is \emph{constant}.
...
\begin{stbox}{General Information}
  \begin{enumerate}
    ...
    \item When faced with atypical probability distributions, sometimes defining a variable \(W\) having a Binomial/Poisson/Geometric distribution and then paraphrasing may help.

    For example, some such relationships include \(M=W-1\), or \(M=W_1+W_2\).
  \end{enumerate}
\end{stbox}
\begin{note}
