...
\begin{note}
  ...
  \begin{enumerate}
    \item Factoring out \(e^{i\theta/2}\) given an expression involving \(e^{i\theta}\).
    \item Finding \(\sin^n(\theta)\) as a linear combination of \(\sin(m\theta)\), where \(m\in \mathbb{Z}^{+}\). first notice that \(z^n-\frac{1}{z^n}=2i\sin(n\theta)\), for \(z=e^{i\theta}\). Then,
    \begin{align*}
      \left( z+\frac{1}{z} \right)^n &= \sum_{r=0}^{n}{\binom{n}{r}z^{n-2r}}\\
      [2i\sin(\theta)]^n &= \cdots
    \end{align*}
    Comparing imaginary parts, we get \(\sin^n(\theta)\) in the desired form. (Similarly for other trigonometric functions raised to the \(n\)th power.)
    \item If, instead, we need to find \(\sin(n\theta)\) in terms of powers of \(\sin(\theta)\), then we simply apply De Moivre's Theorem and compare imaginary parts. (Or real parts, for \(\cos(n\theta)\).) Letting \(c\coloneq\cos(\theta)\) and \(s\coloneq\sin(\theta)\):
    \[\cos(n\theta)+i\sin(n\theta)=(c+is)^n=\sum_{r=0}^{n}{\binom{n}{r}c^{n-r}i^rs^r}.\]
  \end{enumerate}
\end{note}
...
\begin{note}
  ...
  \begin{enumerate}
    \item In circle-line interactions, be aware of any congruent triangles, common angles, and sides of common length. In particular, when a triangle has an angle of \(\pi/4=45^{\circ}\), it is an isosceles triangle. These observations are especially helpful in finding circle-line intersections.
  \end{enumerate}
\end{note}
