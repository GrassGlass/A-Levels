...
\begin{figure}[H]
  ...
  \caption{\ref{source:Student} Student's \(t\)-distribution compared to the standard normal distribution.}
  \label{fig:Student}
\end{figure}
\begin{stbox}{General Information}
  \begin{itemize}
...
\begin{landscape}
  \section{Summary}
Assume that the sample used for each test is random. Square brackets indicate ``and'', while round brackets indicate ``or''. 
  \begin{table}[htbp]
      \begin{tabular}{ScSc}
    ...
    \begin{tabular}{ScSc}
      ...
      \begin{minipage}{418.6pt}
        \begin{enumerate}[align=parleft]
          ...
          \item[{[ii](2)}]\ At least one sample size \(n_1\) or \(n_2\) is small, but we assume \(X_1\) and \(X_2\) are independent and normally distributed.
        \end{enumerate}
      \end{minipage}&
      \begin{minipage}{179.4pt}
        \begin{center}
      ...
      \begin{minipage}{418.6pt}
        \begin{enumerate}[label={[\roman*]},align=parleft]
          ...
          \item Assume \(X_1\) and \(X_2\) are independent and normally distributed.
        \end{enumerate}
