...
\newpage
\section{Conics}
Given a conic section defined by \(Ax^2+Bxy+Cy^2+Dx+Ey+F=0\), we may want to find its lines of symmetry, center, radii, etc. The core idea is simple: complete the square, to express \(Ax^2+Bxy+Cy^2\) in the form \(a(x')^2+b(y')^2\), for some linear combinations \(x'\) and \(y'\) of \(x\) and \(y\). Then, the initial equation becomes \(a(x')^2+b(y')^2+d(x')+e(y')+F=0\), which is easily reduced to the standard form for conics. Before that, we need to develop some machinery.
\begin{definition}{}{}
    Let \(V\) be a vector space over a field \(\mathbb{F}\) not of characteristic two. A function \(K\colon V\to \mathbb{F}\) is called a \emph{quadratic form on \(\mathbb{F}^n\)} if there exists a symmetric matrix \(A\in \operatorname{M}_{n \times n}(\mathbb{F})\), such that \(K(\mathbf{x})=\mathbf{x}^{\top} \mathbf{A}\mathbf{x}\) for all \(\mathbf{x}\in V\).
\end{definition}
\begin{note}
...
\[\begin{pmatrix}
    ...
    B/2 & C
\end{pmatrix}\]
to find it's eigenvalues \(\lambda\) and \(\mu\), then the corresponding unit eigenvectors \(\mathbf{u}=
\begin{pmatrix}
    \alpha & \beta
...
\end{note}
\begin{example}{}{}
    By considering the stationary points of the curve \(y=f(x)\), explain whether \(x_0=\rule{0.5cm}{0.05mm}\) is a suitable starting value for using the Newton-Raphson method to find an approximation to \(\alpha\).
    ...
    \end{figure}
    Since \(x_0=\rule{0.5cm}{0.05mm}\) is very close to the stationary point, the tangent to the curve \(y=f(x)\) is almost a horizontal line. Thus, it cuts the \(x\)-axis far away from the initial approximation. So, \(x_0=\rule{0.5cm}{0.05mm}\) is not a suitable starting value.
\end{example}
\begin{note}
