...
\begin{itemize}
    ...
    \item A longitudinal wave \emph{cannot be polarised} because the direction of oscillation of particles in the wave is already \emph{parallel} to the direction of energy transfer in the wave.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}[c]{0.5\textwidth}
            \centering
            \includegraphics[scale=0.17]{../images/Pipe Closed at One End.jpg}
    ...
    \begin{figure}[H]
        ...
        \end{subfigure}
        \caption{\ref{Me} Double-slit diffraction.}
    ...
    \item \(\text{Total number of dark regions/minimas}=2\mathsf{m}+1\), where \(\mathsf{m}\) is the highest order minima possible.
...
\begin{itemize}
    ...
    \item If there is a conflict of dpsf between the table and the graph, then we follow the one with the lower dpsf.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Calculate the difference in current \(\Delta y_1\) between the current obtained from the graph and the current obtained in (f)(ii).
