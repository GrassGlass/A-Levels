...
\end{itemize}
\begin{center}
    \includegraphics[width=\textwidth]{../images/2024-Ellipse-Map-(cit_).png}
\end{center}
...
\begin{itemize}
  ...
  \item The \emph{eleatic potential energy} of a point charge in an electric field is defined as the \emph{work done by an external agent} in bringing the point charge from infinity to that point (without any net change in kinetic energy). 
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  \item The \emph{electric potential} at a point \(X\) due to a \emph{system} of point charges \(q_i\) is the algebraic sum of the electric potential due to each individual charge \(q_i\) which is distance \(r_i\) away from \(X\). That is, 
  \[V=\sum_{i}^{}{\frac{q_i}{4\pi\varepsilon_0r_i}}.\]
  \item The \emph{potential energy} of a \emph{system} of charges \(q_i\) is the work done to assemble it. This is the sum of energies needed to bring each charge \(q_i\) to the charges \(q_j\) (for \(i>j\)) already present. In other words letting \(r_{ij}\) be the distance of \(q_i\) from \(q_j\), we have 
...
\end{center}
\begin{itemize}
    \item For \(n\) identical resistors in parallel, which are each of resistance \(R\), \(R_{\text{effective}}=R/n\).
    \item Effective resistance is at most the resistance of the smallest resistor, i.e. \(R_{\text{effective}}\leq R_i\) for each \(i\). In fact, the inequality is strict when the number of resistors in parallel \(n\geq 2\).
    ...
    \begin{itemize}
        ...
        \begin{center}
            \includegraphics[width=0.3\columnwidth]{../images/DC-Circuits-Potentiometer-1.jpg}
        \end{center}
...
\begin{itemize}
    ...
    \item The \emph{magnetic flux density} is defined as the \emph{force per unit current per unit length} acting on an \emph{infinitely long current-carrying conductor} placed \emph{perpendicularly} to the magnetic field.
    \item Crosses \(\times\) represents the direction of going ``into the page'', and dots \(\cdot\) represents going ``out of the page''.
    \item Left and right hand rules:
    \begin{center}
        \includegraphics[width=0.4\textwidth]{../images/Left-Right-Hand-Rules/Left-hand-rule.pdf}
        \hspace{2cm}
    ...
    \[B=\frac{\mu_0NI}{2r}.\]
    \begin{center}
        \includegraphics[width=0.5\textwidth]{../images/Flat-Circular-Coil/Flat-Circular-Coil.pdf}
    \end{center}
    ...
    \end{center}
    \item Say we have a straight current-carrying conductor of length \(l\) and current \(I\), which makes an angle \(\theta\) with the magnetic field. For any point on that conductor that experiences a magnetic flux of density \(B\), 
    \[F_B=BIl\sin(\theta).\]
    \item ~
    \begin{center}
        \begin{tabular}{|Sc|Sc|}
    ...
    \end{center}
    \item A charge \(q\) moving at speed \(v\), which makes an angle \(\theta\) with the magnetic flux density \(B\), experiences a magnetic force of magnitude
    \[F_B=qvB\sin(\theta).\]
...
\chapter{}
\begin{center}
    \includegraphics[width=\textwidth]{../images/Transformer/TransformerCropped.pdf}
    % \captionof{figure}{Transformer with $\lWind$~windings on the primary side and $\rWind$~windings on the secondary side.}
\end{center}
\chapter{Bibliography}
\begin{enumerate}
    \item Projectile motion \url{https://tug.org/PSTricks/main.cgi?file=Examples/Physics/physics}
    \item Crane \url{https://tex.stackexchange.com/a/158785}
    \item Simple harmonic motion \url{https://tex.stackexchange.com/a/158741}
    \item 2024 Ellipse Map \url{https://discord.com/channels/268882317391429632/359052581022203914/1221402243039887451}
    ...
    \item Magnetic field produced by a bar magnet \url{https://tex.stackexchange.com/a/470755} 
    \item Current in a wire \url{https://tikz.net/magnetic_field_wire/}
    \item Left and right hand rules \url{https://tikz.net/righthand_rule/}
    \item Flat circular coil \url{https://tex.stackexchange.com/a/523072}
    \item Solenoid \url{https://en.wikipedia.org/wiki/Lenz's_law#/media/File:VFPt_Solenoid_correct2.svg}
    \item Two current carrying conductors \url{https://tikz.net/magnetic_field_wire_force/}
    \item Electron deflection \url{https://tug.org/PSTricks/main.cgi?file=Examples/Physics/physics}
    \item Transformer \url{https://tex.stackexchange.com/a/158815}
    \item Electromagnetic Wave: \url{https://tug.org/PSTricks/main.cgi?file=Examples/Physics/physics}
\end{enumerate}
\end{document}