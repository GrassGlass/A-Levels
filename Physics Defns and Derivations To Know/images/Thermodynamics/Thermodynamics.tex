% Source: https://tikz.net/thermodynamics_pv_diagrams/
...
\begin{tikzpicture}
  ...
  \fill[mylightblue] (A) rectangle (C|-0,0) node[midway,blue] {\(W<0\)};
...
\begin{tikzpicture}
    ...
    \coordinate (B) at (\Vc,{\isotherm{\Vc}{\Tc}});
    \coordinate (C) at (\Vh,{\isotherm{\Vh}{\Tc}});
    ...
    \draw[mydarkblue,thick,
      ...
      plot (\x,\isotherm{\x}{\Tc});
      \draw[blue,thick,
      domain={0.1*\xmax}:{0.3*\xmax},samples=\N,smooth,-{stealth}]
      plot (\x,\isotherm{\x}{\Tc});
    \draw[blue,thick,
    domain={0.3*\xmax}:{0.7*\xmax},samples=\N,smooth,]
      plot (\x,\isotherm{\x}{\Tc});
...
\begin{tikzpicture}
  ...
  % PATHS
  \draw[pink!75!red,thick,midarr=.48] (A) -- (B);
  \draw[green!85!black,thick,midarr=.58] (A) -- (C);
