...
\begin{tikzpicture}
  ...
  \draw[force] (0.25*\mw,\my+0.7*\mh) --++ (0,\mh) node[left] {\(\rho_\ell gh_2A_2\)};
  \draw[force] (0.25*\mw,\my+0.2*\mh) --++ (0,-0.7*\mh) node[above=-5.5,right=-13] {\(\rho_\ell gh_1A_1\)};
  \draw[force] (-0.3*\mw,\my+0.3*\mh) --++ (0,-1.1*\mh) node[above=2,left=0] {$mg$};
  
  % AXIS depth
  \draw[->,thick]
    (-0.64*\W,\H) --++ (0,-1.1*\H) node[left] {\(h\)};
...
\begin{tikzpicture}
  ...
  \draw[{Latex[length=2.5,width=2.5]}-{Latex[length=2.5,width=2.5]}]
    (-0.55*\W,\h) --++ (0,{\my-\h}) node[midway,left=0] {\(h\)};
  \draw[mass] (-\mw/2,\my) rectangle++ (\mw,\mh) node[midway,above=-5] {$m$};
  \draw[force] (0.30*\mw,\my+0.7*\mh) --++ (0,\F) node[below=4,left=0] {\small Upthrust};
  \draw[force] (-0.3*\mw,\my+0.2*\mh) --++ (0,-0.7*\F) node[above=2,left=0] {\(mg\)};
